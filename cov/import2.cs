...
async Task Main()
{
	...
	using (okWriter = new StreamWriter(@"c:\Users\petrk\Downloads\wikidata-covid-mapping-success-2.csv", false, Encoding.UTF8))
	using (failWriter = new StreamWriter(@"c:\Users\petrk\Downloads\wikidata-covid-mapping-errors-2.csv", false, Encoding.UTF8))
	{
		using (var inputFile = new StreamReader(@"c:\Users\petrk\Downloads\wikidata-covid-mapping-errors.csv", Encoding.UTF8))
		{
			string line;
			while ((line = inputFile.ReadLine()) != null)
			{
				...
				var name = NameToUrlFragment(fullName);
		...
		foreach (var athlete in namesToProcess.Values)
		{
			okWriter.Flush();
			failWriter.Flush();
			if (problematicNames.Contains(NameToUrlFragment(athlete.FullName)))
			{
...
async Task ProcessIdFromName(Athlete athlete)
{
	...
	var url = $"https://www.olympijskytym.cz/athlete/{NameToUrlFragment(athlete.FullName)}";
